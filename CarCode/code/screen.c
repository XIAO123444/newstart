...
//        ips200_draw_point((uint16)centerline[i], i+140, RGB565_PURPLE);//紫色中线
        ips200_draw_point((uint16)leftfollowline[i], i+180, RGB565_RED);//红色左线
        ips200_draw_point((uint16)rightfollowline[i], i+180, RGB565_BLUE);//蓝色右线
        ips200_draw_point((uint16)centerline2[i], i+180, RGB565_PURPLE);//紫色中线
    }
    for(int16 i= MT9V03X_H-1; i >= MT9V03X_H-1-left_longest[0]; i --){
        ips200_draw_point((uint16)left_longest[1], i+180, RGB565_GRAY);//黄色左最长白列
    }
    for(int16 i= MT9V03X_H-1; i >= MT9V03X_H-1-right_longest[0]; i --){
        ips200_draw_point((uint16)right_longest[1], i+180, RGB565_GRAY);//黄色右最长白列
    }
    if(showline==true)
    {
