...
int16 raw_leftline[MT9V03X_H];
int16 raw_rightline[MT9V03X_H];
...
int16 search_stop1=0; //终止点,用于圆环元素处理
...
//线点与丢线↓↓↓↓
uint8 raw_leftline_num;         //左线点数量
uint8 raw_rightline_num;        //右线点数量
...
//         centerline[row] = 0.5 * (raw_rightline[row] + raw_leftline[row]);
//     }
// }
//
// ------------------------------------------------------------------------------------------------------------------
...
//             raw_leftline[y] = (int16)(col - mov);
//             raw_leftline_num ++;//左线点计数+
//             break;//找到边界后退出
//         }
//     }
// }
...
//     uint8 mov = 3;//每次作差后的移动量,默认为2，可以根据画面分辨率调整
//     //计算第x行的左边界
//     raw_rightline[y] = MT9V03X_W - 1;//未找到右边界时输出为187
//     for(col = x; col <= MT9V03X_W - mov - 1; col += mov){
//         dif = (float)((mt9v03x_image[y][col] - mt9v03x_image[y][col + mov + 1])<<8);//左移8位即乘256，可避免浮点数乘，加快速度
//         sum = (float)((mt9v03x_image[y][col] + mt9v03x_image[y][col + mov + 1]));
//         sar = fabs(dif / sum);//求取差比和
//         if(sar > sar_thre){//差比和大于阈值代表深浅色突变
//             raw_rightline[y] = (int16)(col + mov);
//             raw_rightline_num ++;//右线点计数+
//             break;//找到边界后退出
//         }
//     }
// }
...
// 分块大津法四个分区阈值（按行列二分：左上、右上、左下、右下）
int16 threshold1 = 0;  // 左上
int16 threshold2 = 0;  // 右上
int16 threshold3 = 0;  // 左下
int16 threshold4 = 0;  // 右下
/*-------------------------------------------------------------------------------------------------------------------
  @brief     快速大津求阈值，来自山威
  @param     image       图像数组
             col         列 ，宽度
...
/*-------------------------------------------------------------------
  @brief    基于 ROI 的大津阈值计算（与 my_adapt_threshold 同策略：隔行隔列抽样）
    @param    image       图像数组
                img_w       图像宽度
                img_h       图像高度
                x0          ROI 左上角 X 坐标
                            ...
                            uint16 x0, uint16 y0, uint16 w, uint16 h)
{
    ...
    // 夹紧到全局上下限（来自外部可调范围）
    // extern int16 threshold_down; // 已在本文件顶部声明为 extern
...
// 将整幅图像按（行/列）二分为 4 块，分别计算阈值到 threshold1..4
...
#define GrayScale 256
    ...
    // left_max_gray 为左区域出现频率最高的灰度值
    // right_max_gray 为右区域出现频率最高的灰度值
...
void difsum_right1(uint8 y,uint8 x)
{
    ...
    uint8 mov = 1;//每次作差后的移动量,默认为2，可以根据画面分辨率调整
    //计算第x行的左边界
    raw_rightline[y] = MT9V03X_W-1;//未找到左边界时输出为0
    for(col = x; col <= MT9V03X_W - mov - 1; col += mov)
	{
		if(dis_image[y][col] - dis_image[y][col + mov]>5)
		{
			raw_rightline[y] = col ;
			raw_rightline_num ++;//右边线点计数+
			return;//找到边界后退出
		}
...
void param_init(void)
{
    ...
    for(int16 i=0;i<MT9V03X_H;i++)
    {
        raw_leftline[i]=0;              //左线清0
        raw_rightline[i]=MT9V03X_W-1;   //右线清0
...
void image_boundary_process2(void)
    ...
    else
    {
      ...
      //测赛宽
    // printf("\n");
    // printf("raw_rightline");

    // for(int16 i=0;i<MT9V03X_H;i++)
    // {
    //     printf("%d,",raw_rightline[i]);
    // }
    // printf("\n");
    // printf("raw_leftline");
    // for(int16 i=0;i<MT9V03X_H;i++)
    // {
    //     printf("%d,",raw_leftline[i]);
    // }      
    for(row = MT9V03X_H - 1; row > search_stop; row--)
    {
        ...
        if(boundry_start_left==0&&raw_leftline[row]!=0) {boundry_start_left=row;}   //左边界起始点
        if(boundry_start_right==0&&raw_rightline[row]!=MT9V03X_W-1) {boundry_start_right=row;} //右边界起始点
    }
}
...
//十字判断
/*-------------------------------------------------------------------------------------------------------------------
  @brief     计算左右边界的差分，供拐点使用
  @param     无
  @return    修改两个全局数组
             delta_leftline[];
             delta_rightline[];
  Sample     find_tubian_point();
  @note      无
-------------------------------------------------------------------------------------------------------------------
*/
...
#define THRESHOLD (3)
#define MAX_POINTS (5)
Point point_L_down[MAX_POINTS];//左边界下角点
Point point_R_down[MAX_POINTS];//右边界下角点
Point point_L_up[MAX_POINTS];//左边界上角点
Point point_R_up[MAX_POINTS];//右边界上角点

uint8 num_left_up=0;         //左边界有效角点数量
uint8 num_right_up=0;        //右边界有效角点数量
uint8 num_left_down=0;       //左边界有效角点数量
uint8 num_right_down=0;      //右边界有效角点数量
...
void tubian_point_init(void)
{
    ...
    num_left_up=0;         //左边界有效角点数量
    num_right_up=0;        //右边界有效角点数量
    num_left_down=0;       //左边界有效角点数量
    num_right_down=0;      //右边界有效角点数量
}
...
void find_tubian_point(void)
{
    ...
    //直道左列>0，右列小于0，凹变左极大右极小，凸变左极大右极小
    for(;i<=MT9V03X_H-2;i++)
    {
...
{
    int16 i;
    uint8 countldown=0;//左下角点数
    uint8 countlup=0;//左上角点数
    uint8 countrdown=0;//右下角点数
    uint8 countrup=0;//右上角点数

    int8 templ=0;//记录上下角点防止重复记录，1为上角点-1为下角点
    int8 tempr=0;
    for(i=MT9V03X_H-1;i>=search_stop+2;i--)//找下角点
    {   
...
void debugger_jiaopoint_value(void)
{
    ...
    for(int16 i=num_left_down;i<MAX_POINTS;i++)
    {
        ...
        ips200_show_string(32,20*i,"ivd");//invalid数据
        ips200_set_color(RGB565_WHITE,RGB565_BLACK);
    }
    ...
    {
        ips200_set_color(RGB565_RED,RGB565_BLACK);
        ips200_show_string(88,20*i,"ivd");//invalid数据
        ips200_set_color(RGB565_WHITE,RGB565_BLACK);
    }
    ...
    {
        ips200_set_color(RGB565_RED,RGB565_BLACK);
        ips200_show_string(144,20*i,"ivd");//invalid数据
        ips200_set_color(RGB565_WHITE,RGB565_BLACK);
    }
    ...
    {
        ips200_set_color(RGB565_RED,RGB565_BLACK);
        ips200_show_string(200,20*i,"ivd");//invalid数据
        ips200_set_color(RGB565_WHITE,RGB565_BLACK);
    }
...
void Find_Down_Point(int16 start,int16 end)
{
    ...
    for(i=start;i>=end;i--)
    {
        if(Left_Down_Find==0&&//只找第一个符合条件的点
            (raw_leftline[i]>0)&&//左边界点不能为0
           abs(raw_leftline[i]-raw_leftline[i+1])<=3&&//角点的阈值可以更改
           abs(raw_leftline[i+1]-raw_leftline[i+2])<=3&&
           abs(raw_leftline[i+2]-raw_leftline[i+3])<=3&&
            ((raw_leftline[i]-raw_leftline[i-2])>=3)&&
            ((raw_leftline[i]-raw_leftline[i-3])>=5)&&
            ((raw_leftline[i]-raw_leftline[i-4])>=5))
        {
        ...
        if(Right_Down_Find==0&&//只找第一个符合条件的点
           abs(raw_rightline[i]-raw_rightline[i+1])<=3&&//角点的阈值可以更改
           abs(raw_rightline[i+1]-raw_rightline[i+2])<=3&&
           abs(raw_rightline[i+2]-raw_rightline[i+3])<=3&&
            raw_rightline[i]<MT9V03X_W-1&&//右边界点不能为MT9V03X_W-1
              ((raw_rightline[i]-raw_rightline[i-2])<=-3)&&
              ((raw_rightline[i]-raw_rightline[i-3])<=-5)&&
              ((raw_rightline[i]-raw_rightline[i-4])<=-5))
        {
            Right_Down_Find=i+2;
...
void Find_Up_Point(int16 start,int16 end)
{
    ...
    { 
        if(Left_Up_Find==0&&//只找第一个符合条件的点
           abs(raw_leftline[i]-raw_leftline[i-1])<=3&&
           abs(raw_leftline[i-1]-raw_leftline[i-2])<=3&&
           abs(raw_leftline[i-2]-raw_leftline[i-3])<=3&&
           raw_leftline[i-2]-raw_leftline[i]>-2&&
            raw_leftline[i-3]-raw_leftline[i]>-3&&
              ((raw_leftline[i]-raw_leftline[i+2])>=3)&&
              ((raw_leftline[i]-raw_leftline[i+3])>=5)&&
              ((raw_leftline[i]-raw_leftline[i+4])>=10))
        {
        ...
        if(Right_Up_Find==0&&//只找第一个符合条件的点
           abs(raw_rightline[i]-raw_rightline[i-1])<=3&&//下面两行位置差不多
           abs(raw_rightline[i-1]-raw_rightline[i-2])<=3&&
           abs(raw_rightline[i-2]-raw_rightline[i-3])<=3&&
           raw_rightline[i-2]-raw_rightline[i]<2&&
           raw_rightline[i-3]-raw_rightline[i]<3&&
              ((raw_rightline[i]-raw_rightline[i+2]<=-3))&&
              ((raw_rightline[i]-raw_rightline[i+3])<=-5)&&
              ((raw_rightline[i]-raw_rightline[i+4])<=-10))
        {
...
int16 Find_Left_Down_Point(int16 start,int16 end)//找左下角点，返回值是角点所在的行数
 {
    ...
    if(raw_leftline[start]<=0&&raw_leftline[start-1]<=0&&raw_leftline[start-2]<=0&&raw_leftline[start-3]<=0)//如果起始点就都丢线，没有拐点判断的意义
    {
        return 0;
    }
    for(int i=start;i>=end;i--)//由下至上找角点 
    {
        if(((raw_leftline[i]-raw_leftline[i-2])>=8||raw_leftline[i-2]==0)&&
           ((raw_leftline[i]-raw_leftline[i-3])>=15||raw_leftline[i-3]==0)&&
           ((raw_leftline[i]-raw_leftline[i-4])>=15||raw_leftline[i-4]==0))
        {
...
int16 Find_Right_Down_Point(int16 start,int16 end)
{
    ...
    if(raw_rightline[start]>=MT9V03X_W-1&&raw_rightline[start-1]>=MT9V03X_W-1&&raw_rightline[start-2]>=MT9V03X_W-1&&raw_rightline[start-3]>=MT9V03X_W-1)//如果起始点就都丢线，没有拐点判断的意义
    {
        return 0;
    }
    for(int i=start;i>=end;i--)
    {
        if(raw_rightline[i]<=raw_rightline[i+1]&&
            raw_rightline[i+1]<=raw_rightline[i+2]&&
            ((raw_rightline[i]-raw_rightline[i-2])<=-8)&&
           ((raw_rightline[i]-raw_rightline[i-3])<=-15||raw_rightline[i-3]==MT9V03X_W-1)&&
           ((raw_rightline[i]-raw_rightline[i-4])<=-15||raw_rightline[i-4]==MT9V03X_W-1))
        {
...
int16 Find_Right_Up_Point(int16 start,int16 end)//找四个角点，返回值是角点所在的行数
{
    ...
    {end=MT9V03X_H-1-4;}
    if(raw_rightline[start]==MT9V03X_W-1&&raw_rightline[start+1]==MT9V03X_W-1&&raw_rightline[start+2]==MT9V03X_W-1)
    {
        return 0;//如果起始点就都丢线，没有拐点判断的意义
    }
    for(i=start;i<=end;i++)
    {
        if(raw_rightline[i]==MT9V03X_W-1)//如果右边界点是MT9V03X_W-1，说明没有找到
        {
            break;  //直接作废
        }
        if(raw_rightline[i]>=raw_rightline[i-1]&&
...
int16 Find_Left_Up_Point(int16 start,int16 end)//找四个角点，返回值是角点所在的行数
{
    ...
    if(end>=MT9V03X_H-1-4)
    {end=MT9V03X_H-1-4;}
    if(raw_leftline[start]==0&&raw_leftline[start+1]==0&&raw_leftline[start+2]==0)
    {
        return 0;//如果起始点就都丢线，没有拐点判断的意义
    }
    for(i=start;i<=end;i++)//由上至下找角点
    {
        if(((raw_leftline[i]-raw_leftline[i+2])>=8)&&
           ((raw_leftline[i]-raw_leftline[i+3])>=15||raw_leftline[i+3]==0)&&
           ((raw_leftline[i]-raw_leftline[i+4])>=15||raw_leftline[i+4]==0))
        {
...
int16 continuity_right(uint8 start,uint8 end)
{
    ...
    for(i=start;i>=end;i--)
    {
        if(abs(raw_rightline[i]-raw_rightline[i-1])>=5)//连续性阈值是5，可更改
       {
...
int16 continuity_left(uint8 start,uint8 end)
{
    ...
    for(i=start;i>=end;i--)
    {
        if(abs(raw_leftline[i]-raw_leftline[i-1])>=5)//连续性阈值是5，可更改
       {
...
{            
    ...
    for(i=end;i<=start;i++)
    {
        ...
        if(raw_rightline[i] <raw_rightline[i+5]&&raw_rightline[i] <raw_rightline[i-5]&&raw_rightline[i+1]!=MT9V03X_W-1&&raw_rightline[i-1]!=MT9V03X_W-1)
        {
            result=i;
            return result;
        }
...
int16 montonicity_left(uint8 start, uint8 end) 
{
    ...
    for (i = end; i <= start; i++) {
        ...
        if (raw_leftline[i] > raw_leftline[i + 5] && raw_leftline[i] > raw_leftline[i - 5]) {
            result = i;
            return result;
        }
    }
...
void lenthen_Left_bondarise(int16 start)
{
    ...
    float dx=(float)(raw_leftline[start]-raw_leftline[start-5])/5;
    dx1_left_average(dx);
    float dx_average=(dx1[0]+dx1[1]+dx1[2]+dx1[3]+dx1[4])/5;
    for(int16 i=start;i<MT9V03X_H-1;i++)
    {
        if((float)raw_leftline[start]+(float)(dx_average*(i-start))<0)
        {
            leftfollowline[i]=0;
        }
        else if((float)raw_leftline[start]+dx_average*(float)(i-start)>(float)MT9V03X_W-2)
        {
            leftfollowline[i]=MT9V03X_W-1;
        }
        else
        {
            leftfollowline[i]=(int16)((float)raw_leftline[start]+dx_average*(float)(i-start));
        }
    }
}
 /*
...
void lenthen_Right_bondarise(int16 start)  
{
    if(start<7){start=7;}
    if(start>MT9V03X_H-1){start=MT9V03X_H-1;}
    float dx=(float)(raw_rightline[start]-raw_rightline[start-5])/5;
    dx2_right_average(dx);
    float dx_average=(dx2[0]+dx2[1 ]+dx2[2]+dx2[3]+dx2[4])/5;
    for(int16 i=start;i<MT9V03X_H-1;i++)
    {
        if((float)raw_rightline[start]+dx_average*(i-start)>MT9V03X_W-2)
        {
            rightfollowline[i]=MT9V03X_W-1;
        }
        else if((float)raw_rightline[start]+dx_average*(float)(i-start)<0)
        {
            rightfollowline[i]=0;
        }
        else 
        {
            rightfollowline[i]=(int16)((float)raw_rightline[start]+dx_average*(float)(i-start));
        }
    }
}
/*
...
void lenthen_Left_bondarise_bottom(int16 start)
{
    ...
    {
        if((float)raw_leftline[start]+(float)(dx*(i-start))<0)
        {
            leftfollowline[i]=0;
        }
        else if((float)raw_leftline[start]+dx*(float)(i-start)>(float)MT9V03X_W-2)
        {
            leftfollowline[i]=MT9V03X_W-1;
        }
        else
        {
            leftfollowline[i]=(int16)((float)raw_leftline[start]+dx*(float)(i-start));
        }
    }
}
...
void lenthen_Right_bondarise_bottom(int16 start)  
{
    ...
    for(int16 i=start;i>=0;i--)
    {
        if((float)raw_rightline[start]+dx*(i-start)>MT9V03X_W-2)
        {
            rightfollowline[i]=MT9V03X_W-1;
        }
        else if((float)raw_rightline[start]+dx*(float)(i-start)<0)
        {
            rightfollowline[i]=0;
        }
        else 
        {
            rightfollowline[i]=(int16)((float)raw_rightline[start]+dx*(float)(i-start));
        }
    }
}
...
void trace_right_bu(int16 start,int16 end)
{
    ...
    for(int16 i=start;i<end;i++)
    {
        int16 temp=raw_leftline[i]+trace_middle[i];
        if(temp>MT9V03X_W-1)
        {
            temp=MT9V03X_W-1;
        }
...
void trace_left_bu(int16 start,int16 end)
{
    ...
    for(int16 i=start;i<end;i++)
    {
        int16 temp=raw_rightline[i]-trace_middle[i];
        if(temp<0)
        {
            temp=0;
        }
...
{   
    ...
    my_adapt_threshold_block4(mt9v03x_image[0], MT9V03X_W, MT9V03X_H); // 分区大津法获取
